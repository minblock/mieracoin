...
const char * const BITCOIN_CONF_FILENAME = "mieracoin.conf";
...
#else
    const char* pszModule = "mieracoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mieracoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mieracoin";
#else
    // Unix
    return pathRet / ".mieracoin";
#endif
#endif
}
